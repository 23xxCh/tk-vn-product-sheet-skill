...
def _configure_language(target_language: str, language_code: str) -> None:
    global TARGET_LANGUAGE, LANGUAGE_CODE, PROMPT, TRANSLATE_SYSTEM
    TARGET_LANGUAGE = target_language
    LANGUAGE_CODE = language_code
    PROMPT = (
        "Remove every brand name, logo, shop mark and watermark. "
        f"Translate all source-language text to natural {target_language} ({language_code}). "
        "Preserve the product, numeric specifications, units, layout and aspect ratio exactly."
    )
    TRANSLATE_SYSTEM = (
        f"你是TikTok跨境电商翻译专家。把给定JSON翻译成{target_language}({language_code})，规则：\n"
        "1. title：目标语言的自然电商语序，≤80字符；删除原装/原厂/正品/专柜/官方等侵权暗示。\n"
        "2. 品牌仅可作为兼容对象，使用目标语言自然的“适用于/兼容”表达，不得作为商品主体。\n"
        "3. vname/vval：去品牌并本地化；保留数字、尺寸、技术参数和单位。\n"
        "4. desc_text：去品牌和无关营销信息，翻译为目标语言，保留<p>等HTML结构。\n"
        "只输出JSON，键与输入完全一致，空值保持空。"
    )


_configure_language(TARGET_LANGUAGE, LANGUAGE_CODE)
...
def auto_process(xlsx_path: str, ark_key: str, hfsy_key: str, agnes_key: str,
    ...
    def translate_row(row: dict, key: str) -> dict:
        ...
        if not src:
            return {"row_index": r, "translate": {}, "desc_text_target": ""}
        ...
        for key_name, col in [("title", "B"), ("vname1", "G"), ("vval1", "H"),
            ...
            if vi.get(key_name):
                tr[col] = vi[key_name]
        return {"row_index": r, "translate": tr, "desc_text_target": vi.get("desc_text", "")}
    ...
    if ark_keys:
        ...
        for row in w["rows"]:
            ...
            if t:
                ...
                if t["desc_text_target"]:
                    row["desc_text_target"] = t["desc_text_target"]
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(description="Auto batch process multilingual TikTok product sheet")
    ...
    ap.add_argument("--target-language", default="Vietnamese")
    ap.add_argument("--language-code", default="vi")
    args = ap.parse_args(argv)
    _configure_language(args.target_language, args.language_code)
    ...
    print("=== TikTok Multilingual Product Sheet Auto Processor ===", flush=True)
    print(f"Target language: {args.target_language} ({args.language_code})", flush=True)
