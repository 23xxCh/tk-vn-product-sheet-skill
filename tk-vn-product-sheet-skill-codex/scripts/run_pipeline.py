"""Orchestrator for the tk-vn product sheet pipeline.
  ...
  finalize <xlsx> <work.json> <out.xlsx>
      ...
      (localized title/variants, replaced image URLs, weight, dims, rewritten
...
def prepare(xlsx_path: str, work_json: str) -> None:
    ...
    for r in range(2, ws.max_row + 1):
        ...
        "desc_text_target": "",            # agent填: 去品牌+本地化后的文字HTML
        "desc_text_vi": "",                # 兼容旧版work.json
        "images": images,
        })
...
def finalize(xlsx_path: str, work_json: str, out_xlsx: str) -> None:
    ...
    for row in work["rows"]:
        ...
        for img in row["images"]:
            ...
            if dec == "delete":
                ...
                # Listing images are required inventory slots and must never be deleted.
                continue
            if dec == "regen" and new_url:
                if not str(new_url).startswith("https://"):
                    raise ValueError(
                        f"row {r} {img['col']}: regenerated image must be a public HTTPS URL"
                    )
                if img["col"] == COL["desc"]:
        ...
        target_text = row.get("desc_text_target") or row.get("desc_text_vi") or ""
        if not target_text:
            target_text = sheet_io.extract_text_content(cur_html)
        ws.cell(row=r, column=desc_ci).value = sheet_io.build_description_all(target_text, new_urls)

        # physical attrs
        collected_weight = row.get("weight_kg", collected_weight)
        collected_l = row.get("l", collected_l)
        collected_w = row.get("w", collected_w)
        collected_h = row.get("h", collected_h)
    ...
    # Keep exactly the 35-column TikTok output schema.
    if ws.max_column > 35:
        ws.delete_cols(36, ws.max_column - 35)
    headers = [
        "*分类id\n（必填）", "*产品标题\n（必填）", "*产品描述\n（必填）", "品牌",
        "产品属性", "SKU", "变种属性名称一", "变种属性值一", "变种属性名称二",
        "变种属性值二", "变种属性名称三", "变种属性值三", "识别码类型", "识别码",
        "*本地展示价(站点币种)\n（必填）", "*库存\n（必填）",
        "*产品主图(URL)地址\n（必填）", "附图一", "附图二", "附图三", "附图四",
        "附图五", "附图六", "附图七", "附图八", "视频链接", "尺码图",
        "变种主题1图片", "*重量(kg)\n（必填）", "*长(cm)\n（必填）",
        "*宽(cm)\n（必填）", "*高(cm)\n（必填）", "*仓库名称\n（必填）",
        "货到付款", "来源URL",
    ]
